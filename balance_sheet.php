<?php
session_start();
require_once 'config/database.php';
require_once 'classes/Accounting.php';

$accounting = new Accounting($conn);

$asOfDate = $_GET['date'] ?? date('Y-m-d');
$balanceSheet = $accounting->generateBalanceSheet($asOfDate);

// Get date range
$startDate = isset($_GET['start_date']) ? $_GET['start_date'] : date('Y-m-01');
$endDate = isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-t');

// Get asset accounts
$sql = "SELECT 
            coa.account_id,
            coa.account_code,
            coa.account_name,
            COALESCE(SUM(CASE 
                WHEN jed.debit_amount > 0 THEN jed.debit_amount 
                ELSE 0 
            END), 0) as total_debit,
            COALESCE(SUM(CASE 
                WHEN jed.credit_amount > 0 THEN jed.credit_amount 
                ELSE 0 
            END), 0) as total_credit
        FROM chart_of_accounts coa
        LEFT JOIN journal_entry_details jed ON coa.account_id = jed.account_id
        LEFT JOIN journal_entries je ON jed.entry_id = je.entry_id
        WHERE coa.is_active = 1
        AND coa.account_type = 'asset'
        AND (je.entry_date IS NULL OR je.entry_date BETWEEN ? AND ?)
        GROUP BY coa.account_id
        ORDER BY coa.account_code";

$stmt = $conn->prepare($sql);
$stmt->bind_param("ss", $startDate, $endDate);
$stmt->execute();
$assets = $stmt->get_result();

// Get liability accounts
$sql = "SELECT 
            coa.account_id,
            coa.account_code,
            coa.account_name,
            COALESCE(SUM(CASE 
                WHEN jed.debit_amount > 0 THEN jed.debit_amount 
                ELSE 0 
            END), 0) as total_debit,
            COALESCE(SUM(CASE 
                WHEN jed.credit_amount > 0 THEN jed.credit_amount 
                ELSE 0 
            END), 0) as total_credit
        FROM chart_of_accounts coa
        LEFT JOIN journal_entry_details jed ON coa.account_id = jed.account_id
        LEFT JOIN journal_entries je ON jed.entry_id = je.entry_id
        WHERE coa.is_active = 1
        AND coa.account_type = 'liability'
        AND (je.entry_date IS NULL OR je.entry_date BETWEEN ? AND ?)
        GROUP BY coa.account_id
        ORDER BY coa.account_code";

$stmt = $conn->prepare($sql);
$stmt->bind_param("ss", $startDate, $endDate);
$stmt->execute();
$liabilities = $stmt->get_result();

// Get equity accounts
$sql = "SELECT 
            coa.account_id,
            coa.account_code,
            coa.account_name,
            COALESCE(SUM(CASE 
                WHEN jed.debit_amount > 0 THEN jed.debit_amount 
                ELSE 0 
            END), 0) as total_debit,
            COALESCE(SUM(CASE 
                WHEN jed.credit_amount > 0 THEN jed.credit_amount 
                ELSE 0 
            END), 0) as total_credit
        FROM chart_of_accounts coa
        LEFT JOIN journal_entry_details jed ON coa.account_id = jed.account_id
        LEFT JOIN journal_entries je ON jed.entry_id = je.entry_id
        WHERE coa.is_active = 1
        AND coa.account_type = 'equity'
        AND (je.entry_date IS NULL OR je.entry_date BETWEEN ? AND ?)
        GROUP BY coa.account_id
        ORDER BY coa.account_code";

$stmt = $conn->prepare($sql);
$stmt->bind_param("ss", $startDate, $endDate);
$stmt->execute();
$equities = $stmt->get_result();

// Calculate totals
$totalAssets = 0;
$totalLiabilities = 0;
$totalEquity = 0;

while ($row = $assets->fetch_assoc()) {
    $totalAssets += $row['total_debit'] - $row['total_credit'];
}
$assets->data_seek(0);

while ($row = $liabilities->fetch_assoc()) {
    $totalLiabilities += $row['total_credit'] - $row['total_debit'];
}
$liabilities->data_seek(0);

while ($row = $equities->fetch_assoc()) {
    $totalEquity += $row['total_credit'] - $row['total_debit'];
}
$equities->data_seek(0);

$totalLiabilitiesAndEquity = $totalLiabilities + $totalEquity;
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Balance Sheet - Accounting System</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        <div class="container">
            <a class="navbar-brand" href="index.php">Accounting System</a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link" href="journal_entry.php">Journal Entry</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="ledger.php">Ledger</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="trial_balance.php">Trial Balance</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="income_statement.php">Income Statement</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="balance_sheet.php">Balance Sheet</a>
                    </li>
                </ul>
            </div>
        </div>
    </nav>

    <div class="container mt-4">
        <h2>Balance Sheet</h2>
        
        <form method="GET" class="mb-4">
            <div class="row">
                <div class="col-md-4">
                    <label for="date" class="form-label">As of Date</label>
                    <input type="date" class="form-control" id="date" name="date" 
                           value="<?php echo $asOfDate; ?>" onchange="this.form.submit()">
                </div>
            </div>
        </form>

        <div class="row">
            <div class="col-md-12 mb-4">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0">
                            <i class="bi bi-file-earmark-text me-2"></i>Balance Sheet
                        </h5>
                    </div>
                    <div class="card-body">
                        <form method="GET" class="row g-3 mb-4">
                            <div class="col-md-5">
                                <label class="form-label">Start Date</label>
                                <input type="date" class="form-control" name="start_date" 
                                       value="<?php echo $startDate; ?>" required>
                            </div>
                            <div class="col-md-5">
                                <label class="form-label">End Date</label>
                                <input type="date" class="form-control" name="end_date" 
                                       value="<?php echo $endDate; ?>" required>
                            </div>
                            <div class="col-md-2">
                                <label class="form-label">&nbsp;</label>
                                <button type="submit" class="btn btn-primary w-100">
                                    <i class="bi bi-search me-2"></i>View
                                </button>
                            </div>
                        </form>

                        <div class="row">
                            <!-- Assets -->
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-header bg-primary text-white">
                                        <h6 class="card-title mb-0">
                                            <i class="bi bi-wallet2 me-2"></i>Assets
                                        </h6>
                                    </div>
                                    <div class="card-body">
                                        <div class="table-responsive">
                                            <table class="table table-sm">
                                                <tbody>
                                                    <?php while ($row = $assets->fetch_assoc()): ?>
                                                        <tr>
                                                            <td>
                                                                <strong><?php echo $row['account_code']; ?></strong>
                                                                <br>
                                                                <small class="text-muted"><?php echo $row['account_name']; ?></small>
                                                            </td>
                                                            <td class="text-end">
                                                                <?php echo number_format($row['total_debit'] - $row['total_credit'], 2); ?>
                                                            </td>
                                                        </tr>
                                                    <?php endwhile; ?>
                                                    <tr class="table-light">
                                                        <td class="text-end fw-bold">Total Assets</td>
                                                        <td class="text-end fw-bold"><?php echo number_format($totalAssets, 2); ?></td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <!-- Liabilities and Equity -->
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-header bg-success text-white">
                                        <h6 class="card-title mb-0">
                                            <i class="bi bi-cash-stack me-2"></i>Liabilities & Equity
                                        </h6>
                                    </div>
                                    <div class="card-body">
                                        <div class="table-responsive">
                                            <table class="table table-sm">
                                                <tbody>
                                                    <!-- Liabilities -->
                                                    <tr class="table-light">
                                                        <td colspan="2" class="fw-bold">Liabilities</td>
                                                    </tr>
                                                    <?php while ($row = $liabilities->fetch_assoc()): ?>
                                                        <tr>
                                                            <td>
                                                                <strong><?php echo $row['account_code']; ?></strong>
                                                                <br>
                                                                <small class="text-muted"><?php echo $row['account_name']; ?></small>
                                                            </td>
                                                            <td class="text-end">
                                                                <?php echo number_format($row['total_credit'] - $row['total_debit'], 2); ?>
                                                            </td>
                                                        </tr>
                                                    <?php endwhile; ?>
                                                    <tr class="table-light">
                                                        <td class="text-end fw-bold">Total Liabilities</td>
                                                        <td class="text-end fw-bold"><?php echo number_format($totalLiabilities, 2); ?></td>
                                                    </tr>

                                                    <!-- Equity -->
                                                    <tr class="table-light">
                                                        <td colspan="2" class="fw-bold">Equity</td>
                                                    </tr>
                                                    <?php while ($row = $equities->fetch_assoc()): ?>
                                                        <tr>
                                                            <td>
                                                                <strong><?php echo $row['account_code']; ?></strong>
                                                                <br>
                                                                <small class="text-muted"><?php echo $row['account_name']; ?></small>
                                                            </td>
                                                            <td class="text-end">
                                                                <?php echo number_format($row['total_credit'] - $row['total_debit'], 2); ?>
                                                            </td>
                                                        </tr>
                                                    <?php endwhile; ?>
                                                    <tr class="table-light">
                                                        <td class="text-end fw-bold">Total Equity</td>
                                                        <td class="text-end fw-bold"><?php echo number_format($totalEquity, 2); ?></td>
                                                    </tr>

                                                    <!-- Total Liabilities and Equity -->
                                                    <tr class="table-primary">
                                                        <td class="text-end fw-bold">Total Liabilities & Equity</td>
                                                        <td class="text-end fw-bold"><?php echo number_format($totalLiabilitiesAndEquity, 2); ?></td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <?php if ($totalAssets !== $totalLiabilitiesAndEquity): ?>
                            <div class="alert alert-danger mt-3">
                                <i class="bi bi-exclamation-triangle me-2"></i>
                                Balance sheet is not balanced! Please check your journal entries.
                            </div>
                        <?php endif; ?>

                        <!-- Summary Cards -->
                        <div class="row mt-4">
                            <div class="col-md-4">
                                <div class="card bg-primary text-white">
                                    <div class="card-body">
                                        <h6 class="card-title">
                                            <i class="bi bi-wallet2 me-2"></i>Total Assets
                                        </h6>
                                        <h3 class="mb-0"><?php echo number_format($totalAssets, 2); ?></h3>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="card bg-success text-white">
                                    <div class="card-body">
                                        <h6 class="card-title">
                                            <i class="bi bi-cash-stack me-2"></i>Total Liabilities
                                        </h6>
                                        <h3 class="mb-0"><?php echo number_format($totalLiabilities, 2); ?></h3>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="card bg-info text-white">
                                    <div class="card-body">
                                        <h6 class="card-title">
                                            <i class="bi bi-graph-up me-2"></i>Total Equity
                                        </h6>
                                        <h3 class="mb-0"><?php echo number_format($totalEquity, 2); ?></h3>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html> 